function  solarpressureforcevector = solarPressureForce(sunlight,panelSurface,...
    ...
    for j=1:size(betas,2) %% pitch
      for i=1:size(alphas,2) %% roll
        ...
        if nozpanels %% zpanel
          ...
          sunforcevectorZ=sunVector(solarPressure, sunlight,normalZ/norm(normalZ), thetasun(i,j,k),panelSurface,nozpanels);
        end
        if noxpanels %% xpanel
          ...
          sunforcevectorX=sunVector(solarPressure, sunlight,normalX/norm(normalX), thetasun(i,j,k),panelSurface,noxpanels);
        end
        if noypanels %% ypanel
          ...
          sunforcevectorY=sunVector(solarPressure, sunlight,normalY/norm(normalY), thetasun(i,j,k),panelSurface,noypanels);
        end
        %%draw
        if draw
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sunforcevectorz2=sunVector(solarPressure, sunlight,normal, theta,panelSurface,noPanels)
  %% model as per:
