...
struct_9iDbyyjfBL0Y1ELTIVXdBC rtP_gains = { 1.0e-5 , 1.0e-10 , 0.01 } ; const
...
rtP_gains ) , GlobalPrm_0 , sizeof ( struct_9iDbyyjfBL0Y1ELTIVXdBC ) ) ; } }
#endif
...
tid ) { obi5idc0yut * dw = ( obi5idc0yut * ) ssGetDWork ( S , 0 ) ; const
real_T * i_peah0d3j3s = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ; const
real_T * i_ow0kcdhlba = ( real_T * ) ssGetInputPortSignal ( S , 1 ) ; const
real_T * i_nn4c3tq5u1 = ( real_T * ) ssGetInputPortSignal ( S , 2 ) ; const
real_T * i_mgrsfcrkpz = ( real_T * ) ssGetInputPortSignal ( S , 3 ) ; const
real_T * i_nl3tjfw4w4 = ( real_T * ) ssGetInputPortSignal ( S , 4 ) ; const
real_T * i_bzjpsz4f3o = ( real_T * ) ssGetInputPortSignal ( S , 5 ) ; const
real_T * i_dgfpst3nus = ( real_T * ) ssGetInputPortSignal ( S , 6 ) ; const
real_T * i_i5b4hs3x4b = ( real_T * ) ssGetInputPortSignal ( S , 7 ) ; const
real_T * i_erhqfldzho = ( real_T * ) ssGetInputPortSignal ( S , 8 ) ; const
real_T * i_lww5lkbioe = ( real_T * ) ssGetInputPortSignal ( S , 9 ) ; const
real_T * i_onpi3z345p = ( real_T * ) ssGetInputPortSignal ( S , 10 ) ; const
real_T * i_ayhiojiy1b = ( real_T * ) ssGetInputPortSignal ( S , 11 ) ;
ACSOutBus * o_B_30_1 = ( ACSOutBus * ) ssGetOutputPortSignal ( S , 0 ) ; if (
tid == 0 ) { asbCubeSatACS ( & ( dw -> rtm ) , i_peah0d3j3s , i_ow0kcdhlba ,
i_nn4c3tq5u1 , i_mgrsfcrkpz , i_nl3tjfw4w4 , i_bzjpsz4f3o , i_dgfpst3nus ,
i_i5b4hs3x4b , i_erhqfldzho , i_lww5lkbioe , i_onpi3z345p , i_ayhiojiy1b ,
o_B_30_1 , & ( dw -> rtb ) , & ( dw -> rtdw ) ) ; jj0if5albh ( & ( dw -> rtb
...
12 , 1 ) ; if ( ! ssSetNumInputPorts ( S , 12 ) ) return ; if ( !
...
( S , 1 , 0.0 ) ; if ( ! ssSetInputPortVectorDimension ( S , 2 , 4 ) ) return
...
ssSetSampleTime ( S , 0 , 1 ) ; ssSetOffsetTime ( S , 0 , 0 ) ;
...
mdlInfo -> genericFcn ; } }
#define MDL_START
static void mdlStart ( SimStruct * S ) { obi5idc0yut * dw = ( obi5idc0yut * )
ssGetDWork ( S , 0 ) ; mdlProcessParameters ( S ) ; ie553oink0 ( & ( dw ->
...
real_T * i_ayhiojiy1b = ( real_T * ) ssGetInputPortSignal ( S , 11 ) ;
ACSOutBus * o_B_30_1 = ( ACSOutBus * ) ssGetOutputPortSignal ( S , 0 ) ; if (
tid == PARAMETER_TUNING_TID ) { asbCubeSatACSTID1 ( ) ; } if ( tid !=
CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { asbCubeSatACS ( & ( dw -> rtm
) , i_peah0d3j3s , i_ow0kcdhlba , i_nn4c3tq5u1 , i_mgrsfcrkpz , i_nl3tjfw4w4
, i_bzjpsz4f3o , i_dgfpst3nus , i_i5b4hs3x4b , i_erhqfldzho , i_lww5lkbioe ,
i_onpi3z345p , i_ayhiojiy1b , o_B_30_1 , & ( dw -> rtb ) , & ( dw -> rtdw ) )
; } }
#define MDL_UPDATE
...
simStateFieldNames ) ; { mxArray * mdlrefDW = mr_asbCubeSatACS_GetDWork (
ssGetDWork ( S , 0 ) ) ; mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } {
mxArray * data = mr_asbCubeSatACS_GetSimStateDisallowedBlocks ( ) ;
